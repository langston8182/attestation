...
import formDataVirginie from '../form-data-virginie.json'
...
export function createForm (attestation) {
  jQuery('#form-profile').empty()
  const form = $('#form-profile')

  const appendToForm = appendTo(form)

  const attestationName = attestation === 'Virginie' ? formDataVirginie : formData
  const formFirstPart = attestationName
    .flat(1)
  ...
  const reasonsData = attestationName
    .flat(1)
